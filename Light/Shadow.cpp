	...
	: values(values)
{
	...
	depthRender->Position(0, 455);
	depthRender->Scale(240, 240);

	values->GlobalLight->Data.Position = D3DXVECTOR3(0, 32, -30);
	...
	//   D3D11_COMPARISON_LESS_EQUAL : shader에서 비교할 때 넘겨준 z값보다 작거나 같은지 비교
...
void Shadow::Erase(wstring name)
{
	for (int i = 0; i < objs.size(); i++)
	{
		if (objs[i]->ModelName() == name)
		{
...
{
	ImGui::SliderInt("Shadow Select", &buffer->Data.Selected, 0, 2);
	ImGui::DragFloat("Shadow Bias", &buffer->Data.Bias, 0.00005f, 0.0001f, 0.1f,"%.4f");
	ImGui::SliderFloat3("Light Positinon", (float*)&values->GlobalLight->Data.Position, -100, 100);
}

void Shadow::Update()
{
	depthRender->Update();
	values->GlobalLight->Data.Direction = D3DXVECTOR3(0, 0, 0) - values->GlobalLight->Data.Position;
