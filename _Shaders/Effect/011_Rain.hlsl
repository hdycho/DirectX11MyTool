...
float4 PS(GeometryOutput input) : SV_TARGET
{
    ...
    float3 rainColor;
    rainColor.x = max(0.3, SunColor.r);
    rainColor.y = max(0.3, SunColor.g);
    rainColor.z = max(0.3, SunColor.b);

    return float4(float3(directionalLight.rgb * directionalLight.a / totalOpacity) * rainColor, totalOpacity);
