...
D3DXCOLOR D3D::backGroundColor = D3DXCOLOR(0, 0, 0, 1);
...
{
	D3D::GetDC()->OMSetRenderTargets(count, rtv, dsv);
}
...
{
	HRESULT hr;
	//RTV => 픽셀의 색상 -> Textrue각각필요
	//DSV => 픽셀의 깊이 -> Textrue각각필요
	
	//그리고 사용자가 사용할 RTV DSV를 따로 만들어야 한다.
	//이 두개의 해상도는 일치해야 한다.


	//기본적으로 가지고 있는 RTV와 DSV를 생성한다
	//Create Backbuffer
	{
		//스왑체인으로부터 시스템에 연결된 백버퍼 정보가져온다 =>백버퍼포인터
		ID3D11Texture2D* backbufferPointer;
		hr = swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void **)&backbufferPointer);
		assert(SUCCEEDED(hr));

		//그다음 스왑체인내에 기본적으로 마련된 텍스처공간을 랜더타겟뷰에 연결한다
	...
	//Create Texture => DSV(Depth Stencil View)용
	{
		...
		desc.MipLevels = 1;
		desc.ArraySize = 1;
		desc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT; // 깊이에 24비트 스텐실에 8비트 사용
