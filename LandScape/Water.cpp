...
void Water::ImGuiRender()
{
	...
	ImGui::BulletText("RunningTime : %.0f", vsBuffer->Data.RunningTime);
	ImGui::Separator();

	ImGui::TextColored(ImVec4(1, 0, 0, 1),"[WaveSetting]");
	...
	ImGui::TextColored(ImVec4(1, 0, 0, 1), "[TextureScale(UV)]");
	ImGui::DragFloat2("TextureScale", vsBuffer->Data.TextureScale);
	ImGui::Separator();

	ImGui::TextColored(ImVec4(1, 0, 0, 1), "[OceanSize]");
	ImGui::DragFloat2("OceanSize", psBuffer->Data.OceanSize, 0.1f);

	ImGui::TextColored(ImVec4(1, 0, 0, 1), "[BumpSetting]");
	ImGui::DragFloat2("BumpSpeed", vsBuffer->Data.BumpSpeed, 0.1f);
	ImGui::DragFloat("BumpHeight", &vsBuffer->Data.BumpHeight, 0.1f);
	ImGui::Separator();

	ImGui::TextColored(ImVec4(1, 0, 0, 1), "[WaterColor]");
	ImGui::ColorEdit4("ShallowColor", psBuffer->Data.ShallowColor);
	ImGui::ColorEdit4("DeepColor", psBuffer->Data.DeepColor);
	ImGui::Separator();

	ImGui::TextColored(ImVec4(1, 0, 0, 1), "[FresnelValues]");
	...
	ImGui::TextColored(ImVec4(1, 0, 0, 1), "[AlphaToHeight]");
	ImGui::DragFloat("ShoreBlend", &psBuffer->Data.ShoreBlend, 0.1f);
	ImGui::DragFloat("HeightRatio", &psBuffer->Data.HeightRatio, 0.1f);
	ImGui::Separator();
	ImGui::Separator();
}
