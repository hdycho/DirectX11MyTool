...
void ParticleManager::InputParticle(wstring name, int count, PTinfo ptInfo, bool isApha)
{
	for (size_t i = 0; i < count; i++)
	{
		ParticleSystem*psystem = new ParticleSystem(ptInfo.textures, ptInfo.animSpeed, isApha);
		psystem->Initialize(ptInfo.velocity, ptInfo.velocityVar, ptInfo.size, ptInfo.perSecontNum, ptInfo.lifeTime, ptInfo.startRange, ptInfo.particleCount);
		psystem->Ready();
...
void ParticleManager::Shot(wstring name, D3DXVECTOR3 pos, D3DXVECTOR3 dir)
{
	...
	if (miter == particleMap.end())return;
	...
	if (particleIdx[name] >= miter->second.size())
		particleIdx[name] = 0;
}
...
void ParticleManager::Update()
{
	...
	for (; miter != particleMap.end(); miter++)
	{
		for (size_t i = 0; i < miter->second.size(); i++)
		{
			...
			if (miter->second[i]->IsEnd())continue;
