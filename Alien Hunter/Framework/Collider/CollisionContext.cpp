	...
	for (UINT i = 0; i < targetLayer->ColliderCount(); i++)
	{
		...
		if (collider->Type() == (UINT)ColliderElement::EType::ESphere)
		{
			...
			if (targetCollider->Type() == (UINT)ColliderElement::EType::ESphere)
			{
				...
				if (resultType == CollisionResult::ResultType::EDestroyOne)
				{
					D3DXMATRIX mat;
					D3DXMatrixIdentity(&mat);

					mat._41 = mat._42 = mat._43 = 100;
					targetCollider->Transform(mat);
				}
			}
		...
		else if (resultType == CollisionResult::ResultType::EDestroyOne)
		{
			if (tempResult->collideCount > 0)
			{
				float distance = FLT_MAX;
...
void CollisionContext::DeleteAll()
{
	for (size_t i = 0; i < colliderLayerContainer.size(); i++)
		colliderLayerContainer[i]->DeleteAll();
}
