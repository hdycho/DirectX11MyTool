...
{
	sceneBounds = new BSphere(D3DXVECTOR3(50, 0, 50), 150);
...
void Shadow::Render()
{
	...
	//depthShadow->Update();
	//depthShadow->Render();
	//ImGui::SliderFloat("rad", &sceneBounds->Radius, 100, 1000);
}
...
void Shadow::Erase(IShadow * shadow)
{
	vector<IShadow*>::iterator viter;

	viter = objs.begin();

	for (; viter != objs.end(); viter++)
	{
		if ((*viter) == shadow)
		{
			objs.erase(viter);
