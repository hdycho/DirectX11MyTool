...
public:
	Effect(wstring file, bool bSubContext = false);
	~Effect();
...
public:
	...
	typedef map<wstring, Effect*>::iterator EffectMapIter;
public:
	static void Delete();
	static void GetEffect(wstring fileName, ID3DBlob** blob, ID3DX11Effect** effect);

	static void PushShader(wstring fileName);
	static Effect*GetShader(wstring fileName);

private:
	static unordered_map<wstring, EffectDesc> effects;
	static map<wstring, Effect*> effectMap;
};