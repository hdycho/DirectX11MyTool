...
void CsResource::CreateSRV(ID3D11Texture2D * resource, ID3D11ShaderResourceView ** srv)
{
	...
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;   //   BufferEx 여야만 structured buffer에 들어감

	HRESULT hr = D3D::GetDevice()->CreateShaderResourceView(resource, &srvDesc, srv);
	assert(SUCCEEDED(hr));
}
