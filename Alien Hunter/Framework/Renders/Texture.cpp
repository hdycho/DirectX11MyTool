...
void Texture::WritePixels(DXGI_FORMAT writeFormat, vector<D3DXCOLOR>& pixels, Texture & copyTex, bool isInteger)
{
#if false
	ID3D11Texture2D* srcTexture;
	view->GetResource((ID3D11Resource **)&srcTexture);
	...
	D3D11_TEXTURE2D_DESC writeTexturedesc;
	ZeroMemory(&writeTexturedesc, sizeof(D3D11_TEXTURE2D_DESC));
	writeTexturedesc.Width = copyTex.metaData.width;
	writeTexturedesc.Height = copyTex.metaData.height;
	writeTexturedesc.MipLevels = copyTex.metaData.mipLevels;
	writeTexturedesc.ArraySize = copyTex.metaData.arraySize;
	writeTexturedesc.Format = writeFormat;
	writeTexturedesc.SampleDesc.Count = 1;
	writeTexturedesc.Usage = D3D11_USAGE_STAGING;
	writeTexturedesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	writeTexturedesc.MiscFlags = copyTex.metaData.miscFlags;
