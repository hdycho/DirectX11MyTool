...
//===========================GeometryData=========================//
GeometryData::GeometryData(XYZAxis axis, Effect*effect, Effect*line, float TransModel)
...
D3DXVECTOR3 GeometryData::Position()
{
	return position;
}

void GeometryData::Position(D3DXVECTOR3 pos)
{
	position = pos;
	UpdateWorld();
}

void GeometryData::Position(float x, float y, float z)
{
	Position(D3DXVECTOR3(x, y, z));
	UpdateWorld();
}

D3DXVECTOR3 GeometryData::Rotation()
{
	return rotate;
}

void GeometryData::Rotation(D3DXVECTOR3 rotate)
{
	this->rotate = rotate;
	UpdateWorld();
}

void GeometryData::Rotation(float x, float y, float z)
{
	Rotation(D3DXVECTOR3(x, y, z));
	UpdateWorld();
}

void GeometryData::RotationDegree(D3DXVECTOR3 rotate)
{
	Rotation(Math::ToRadian(rotate.x), Math::ToRadian(rotate.y), Math::ToRadian(rotate.z));
	UpdateWorld();
}

void GeometryData::RotationDegree(float x, float y, float z)
{
	Rotation(Math::ToRadian(x), Math::ToRadian(y), Math::ToRadian(z));
	UpdateWorld();
}

D3DXVECTOR3 GeometryData::Scale()
{
	return scale;
}

void GeometryData::Scale(D3DXVECTOR3 scale)
{
	this->scale = scale;
	UpdateWorld();
}

void GeometryData::Scale(float x, float y, float z)
{
	Scale(D3DXVECTOR3(x, y, z));
	UpdateWorld();
}

void GeometryData::SetColor(float x, float y, float z)
{
	gizmoColor = { x,y,z,1 };
	line->SetColor(D3DXCOLOR(x, y, z, 1));
}

void GeometryData::SetColor(D3DXCOLOR color)
{
	gizmoColor = color;
	line->SetColor(color);
}

void GeometryData::SetColor(D3DXVECTOR3 color)
{
	gizmoColor = { color.x,color.y,color.z,1 };
	line->SetColor(D3DXCOLOR(color.x, color.y, color.z, 1));
}

void GeometryData::UpdateOnColor()
{
	switch (axisType)
	{
		case GeometryData::XYZAxis::XLine:
			break;
		case GeometryData::XYZAxis::YLine:
			break;
		case GeometryData::XYZAxis::ZLine:
			break;
		case GeometryData::XYZAxis::XAxis:
		case GeometryData::XYZAxis::YAxis:
		case GeometryData::XYZAxis::ZAxis:
			gizmoColor = { 1,1,0,1 };
			line->SetColor(D3DXCOLOR(1, 1, 0, 1));
			break;
		case GeometryData::XYZAxis::XZPlane:
		case GeometryData::XYZAxis::XYPlane:
		case GeometryData::XYZAxis::YZPlane:
			gizmoColor = { 1,1,0,0.7f };
			break;
	}
}

void GeometryData::UpdateOffColor()
{
	switch (axisType)
	{
		case GeometryData::XYZAxis::XLine:
...
void GeometryData::SetLineLength(float length)
{
	lineLength = length;
}

void GeometryData::SetTargetWorld(D3DXMATRIX * target)
{
	targetWorld = target;
}

bool GeometryData::IsPicked()
{
	D3DXVECTOR3 start;
...
void GeometryData::UpdateWorld()
{
	switch (gizmoType)
	{
		case GeometryData::GizmoType::Component:
		{
			...
			Math::toEulerAngle(dRot, dRotate.x, dRotate.y, dRotate.z);

			D3DXMATRIX S, R, T;
			if (geoType == GeometryDataType::AXIS_TYPE)
		...
		break;
		case GeometryData::GizmoType::Own:
		{
			D3DXMATRIX S, R, T;
			D3DXMatrixScaling(&S, scale.x, scale.y, scale.z);
...
void GeometryData::Render()
{
	...
	if (axisType == XYZAxis::XAxis || axisType == XYZAxis::YAxis || axisType == XYZAxis::ZAxis)
	{
		...
		colorVariable->SetFloatVector(gizmoColor);
...
//===========================GeometryData_Scale=========================//
GeometryData_Scale::GeometryData_Scale(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GeometryData(axis, effect, line)
{
	geoType = GeometryDataType::AXIS_TYPE;
...
GeometryData_Scale::~GeometryData_Scale()
{
}

void GeometryData_Scale::Update()
{
	__super::Update();
}

void GeometryData_Scale::Render()
{
	__super::Render();
}
//===========================GeometryData_Scale=========================//
...
//===========================GeometryData_Rotate=========================//
GeometryData_Rotate::GeometryData_Rotate(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GeometryData(axis, effect, line)
{
	geoType = GeometryDataType::AXIS_TYPE;
	this->gizmoType = gizmoType;
...
GeometryData_Rotate::~GeometryData_Rotate()
{
}

void GeometryData_Rotate::Update()
{
	__super::Update();
}

void GeometryData_Rotate::Render()
{
	__super::Render();
}
//===========================GeometryData_Rotate=========================//
...
//===========================GeometryData_Trans=========================//
GeometryData_Trans::GeometryData_Trans(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GeometryData(axis, effect, line)
{
	geoType = GeometryDataType::AXIS_TYPE;
	this->gizmoType = gizmoType;
...
GeometryData_Trans::~GeometryData_Trans()
{
}

void GeometryData_Trans::Update()
{
	__super::Update();
}

void GeometryData_Trans::Render()
{
	__super::Render();
}
//===========================GeometryData_Trans=========================//
...
GeometryData_Plane::GeometryData_Plane(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GeometryData(axis, effect, line, 0)
{
	geoType = GeometryDataType::GEO_TYPE;
	this->gizmoType = gizmoType;
...
GeometryData_Plane::~GeometryData_Plane()
{
}

void GeometryData_Plane::Update()
{
	__super::Update();
}

void GeometryData_Plane::Render()
{
	__super::Render();
	//alphaBuffer->SetPSBuffer(10);
}

GeometryData_Line::GeometryData_Line(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GeometryData(axis, effect, line, 0)
{
	geoType = GeometryDataType::GEO_TYPE;
	this->gizmoType = gizmoType;
...
GeometryData_Line::~GeometryData_Line()
{
}

void GeometryData_Line::Update()
{
	__super::Update();
}

void GeometryData_Line::Render()
{
	__super::Render();
}

GeometryData_Box::GeometryData_Box(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GeometryData(axis, effect, line, 0)
{
	geoType = GeometryDataType::GEO_TYPE;
	this->gizmoType = gizmoType;
...
GeometryData_Box::~GeometryData_Box()
{
}

void GeometryData_Box::Update()
{
	__super::Update();
}

void GeometryData_Box::Render()
{
	//__super::Render();
}
