...
GeometryManager::GeometryManager(float length)
{
	...
	gizmoShader = new Effect(Effects + L"002_Gizmo.hlsl");
	lineShader = new Effect(Effects + L"001_Line.hlsl");
	//ScaleModel
	{
		axisModels[3]->xAxisModel = new GeometryData_Scale(GeometryData::XYZAxis::XAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		...
		axisModels[3]->yAxisModel = new GeometryData_Scale(GeometryData::XYZAxis::YAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		...
		axisModels[3]->zAxisModel = new GeometryData_Scale(GeometryData::XYZAxis::ZAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		...
		axisModels[3]->XZPlane = new GeometryData_Plane(GeometryData::XYZAxis::XZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->XZPlane->SetLineLength(length);
		axisModels[3]->XZPlane->SetTargetWorld(&initMat);

		axisModels[3]->XYPlane = new GeometryData_Plane(GeometryData::XYZAxis::XYPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->XYPlane->SetLineLength(length);
		axisModels[3]->XYPlane->SetTargetWorld(&initMat);

		axisModels[3]->YZPlane = new GeometryData_Plane(GeometryData::XYZAxis::YZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->YZPlane->SetLineLength(length);
		axisModels[3]->YZPlane->SetTargetWorld(&initMat);

		axisModels[3]->xLine = new GeometryData_Line(GeometryData::XYZAxis::XLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->xLine->SetLineLength(length);
		axisModels[3]->xLine->SetTargetWorld(&initMat);

		axisModels[3]->yLine = new GeometryData_Line(GeometryData::XYZAxis::YLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->yLine->SetLineLength(length);
		axisModels[3]->yLine->SetTargetWorld(&initMat);

		axisModels[3]->zLine = new GeometryData_Line(GeometryData::XYZAxis::ZLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->zLine->SetLineLength(length);
		axisModels[3]->zLine->SetTargetWorld(&initMat);

		axisModels[3]->xBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::XBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->xBoundingBox->SetLineLength(length);
		axisModels[3]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[3]->yBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::YBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[3]->yBoundingBox->SetLineLength(length);
		axisModels[3]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[3]->zBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::ZBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
	...
	//ScaleModel
	{
		axisModels[2]->xAxisModel = new GeometryData_Scale(GeometryData::XYZAxis::XAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->xAxisModel->SetLineLength(length);
		axisModels[2]->xAxisModel->SetTargetWorld(&initMat);

		axisModels[2]->yAxisModel = new GeometryData_Scale(GeometryData::XYZAxis::YAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->yAxisModel->SetLineLength(length);
		axisModels[2]->yAxisModel->SetTargetWorld(&initMat);

		axisModels[2]->zAxisModel = new GeometryData_Scale(GeometryData::XYZAxis::ZAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->zAxisModel->SetLineLength(length);
		axisModels[2]->zAxisModel->SetTargetWorld(&initMat);

		axisModels[2]->XZPlane = new GeometryData_Plane(GeometryData::XYZAxis::XZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->XZPlane->SetLineLength(length);
		axisModels[2]->XZPlane->SetTargetWorld(&initMat);

		axisModels[2]->XYPlane = new GeometryData_Plane(GeometryData::XYZAxis::XYPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->XYPlane->SetLineLength(length);
		axisModels[2]->XYPlane->SetTargetWorld(&initMat);

		axisModels[2]->YZPlane = new GeometryData_Plane(GeometryData::XYZAxis::YZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->YZPlane->SetLineLength(length);
		axisModels[2]->YZPlane->SetTargetWorld(&initMat);

		axisModels[2]->xLine = new GeometryData_Line(GeometryData::XYZAxis::XLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->xLine->SetLineLength(length);
		axisModels[2]->xLine->SetTargetWorld(&initMat);

		axisModels[2]->yLine = new GeometryData_Line(GeometryData::XYZAxis::YLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->yLine->SetLineLength(length);
		axisModels[2]->yLine->SetTargetWorld(&initMat);

		axisModels[2]->zLine = new GeometryData_Line(GeometryData::XYZAxis::ZLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->zLine->SetLineLength(length);
		axisModels[2]->zLine->SetTargetWorld(&initMat);

		axisModels[2]->xBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::XBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->xBoundingBox->SetLineLength(length);
		axisModels[2]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[2]->yBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::YBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[2]->yBoundingBox->SetLineLength(length);
		axisModels[2]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[2]->zBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::ZBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
	...
	//RotateModel
	{
		axisModels[1]->xAxisModel = new GeometryData_Rotate(GeometryData::XYZAxis::XAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->xAxisModel->SetLineLength(length);
		axisModels[1]->xAxisModel->SetTargetWorld(&initMat);

		axisModels[1]->yAxisModel = new GeometryData_Rotate(GeometryData::XYZAxis::YAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->yAxisModel->SetLineLength(length);
		axisModels[1]->yAxisModel->SetTargetWorld(&initMat);

		axisModels[1]->zAxisModel = new GeometryData_Rotate(GeometryData::XYZAxis::ZAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->zAxisModel->SetLineLength(length);
		axisModels[1]->zAxisModel->SetTargetWorld(&initMat);

		axisModels[1]->XZPlane = new GeometryData_Plane(GeometryData::XYZAxis::XZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->XZPlane->SetLineLength(length);
		axisModels[1]->XZPlane->SetTargetWorld(&initMat);

		axisModels[1]->XYPlane = new GeometryData_Plane(GeometryData::XYZAxis::XYPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->XYPlane->SetLineLength(length);
		axisModels[1]->XYPlane->SetTargetWorld(&initMat);

		axisModels[1]->YZPlane = new GeometryData_Plane(GeometryData::XYZAxis::YZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->YZPlane->SetLineLength(length);
		axisModels[1]->YZPlane->SetTargetWorld(&initMat);

		axisModels[1]->xLine = new GeometryData_Line(GeometryData::XYZAxis::XLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->xLine->SetLineLength(length);
		axisModels[1]->xLine->SetTargetWorld(&initMat);

		axisModels[1]->yLine = new GeometryData_Line(GeometryData::XYZAxis::YLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->yLine->SetLineLength(length);
		axisModels[1]->yLine->SetTargetWorld(&initMat);

		axisModels[1]->zLine = new GeometryData_Line(GeometryData::XYZAxis::ZLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->zLine->SetLineLength(length);
		axisModels[1]->zLine->SetTargetWorld(&initMat);

		axisModels[1]->xBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::XBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->xBoundingBox->SetLineLength(length);
		axisModels[1]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[1]->yBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::YBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[1]->yBoundingBox->SetLineLength(length);
		axisModels[1]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[1]->zBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::ZBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
	...
	//TransModel
	{
		axisModels[0]->xAxisModel = new GeometryData_Trans(GeometryData::XYZAxis::XAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->xAxisModel->SetLineLength(length);
		axisModels[0]->xAxisModel->SetTargetWorld(&initMat);

		axisModels[0]->yAxisModel = new GeometryData_Trans(GeometryData::XYZAxis::YAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->yAxisModel->SetLineLength(length);
		axisModels[0]->yAxisModel->SetTargetWorld(&initMat);

		axisModels[0]->zAxisModel = new GeometryData_Trans(GeometryData::XYZAxis::ZAxis, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->zAxisModel->SetLineLength(length);
		axisModels[0]->zAxisModel->SetTargetWorld(&initMat);

		axisModels[0]->XZPlane = new GeometryData_Plane(GeometryData::XYZAxis::XZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->XZPlane->SetLineLength(length);
		axisModels[0]->XZPlane->SetTargetWorld(&initMat);

		axisModels[0]->XYPlane = new GeometryData_Plane(GeometryData::XYZAxis::XYPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->XYPlane->SetLineLength(length);
		axisModels[0]->XYPlane->SetTargetWorld(&initMat);

		axisModels[0]->YZPlane = new GeometryData_Plane(GeometryData::XYZAxis::YZPlane, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->YZPlane->SetLineLength(length);
		axisModels[0]->YZPlane->SetTargetWorld(&initMat);

		axisModels[0]->xLine = new GeometryData_Line(GeometryData::XYZAxis::XLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->xLine->SetLineLength(length);
		axisModels[0]->xLine->SetTargetWorld(&initMat);

		axisModels[0]->yLine = new GeometryData_Line(GeometryData::XYZAxis::YLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->yLine->SetLineLength(length);
		axisModels[0]->yLine->SetTargetWorld(&initMat);

		axisModels[0]->zLine = new GeometryData_Line(GeometryData::XYZAxis::ZLine, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->zLine->SetLineLength(length);
		axisModels[0]->zLine->SetTargetWorld(&initMat);

		axisModels[0]->xBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::XBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->xBoundingBox->SetLineLength(length);
		axisModels[0]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[0]->yBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::YBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
		axisModels[0]->yBoundingBox->SetLineLength(length);
		axisModels[0]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[0]->zBoundingBox = new GeometryData_Box(GeometryData::XYZAxis::ZBox, gizmoShader, lineShader, GeometryData::GizmoType::Component);
...
GeometryManager::~GeometryManager()
{
	for (int i = 0; i < 4; i++)
	{
		...
		SAFE_DELETE(axisModels[i]->xBoundingBox);
		SAFE_DELETE(axisModels[i]->yBoundingBox);
		SAFE_DELETE(axisModels[i]->zBoundingBox);
