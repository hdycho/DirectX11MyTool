...
protected:
	...
	float transModel;	//S,R,T모델은 이동한상태로 처리해야 하므로 1
...
private:
	...
	void BoxInit();
public:
	GeometryData(XYZAxis axis, Effect*effect, class Effect*line, float TransModel = 1.0f);
...
//피킹기능 추가해야함
//모델에 해당하는 라인그리는것도 추가
class GeometryData_Scale :public GeometryData
{
public:
	GeometryData_Scale(XYZAxis axis, Effect*effect, class Effect*line, GizmoType gizmoType);
	~GeometryData_Scale();
...
class GeometryData_Rotate :public GeometryData
{
public:
	GeometryData_Rotate(XYZAxis axis, Effect*effect, class Effect*line, GizmoType gizmoType);
	~GeometryData_Rotate();
...
class GeometryData_Trans :public GeometryData
{
public:
	GeometryData_Trans(XYZAxis axis, Effect*effect, class Effect*line, GizmoType gizmoType);
	~GeometryData_Trans();
...
class GeometryData_Plane :public GeometryData
{
public:
	GeometryData_Plane(XYZAxis axis, Effect*effect, class Effect*line, GizmoType gizmoType);
	~GeometryData_Plane();
...
class GeometryData_Line :public GeometryData
{
public:
	GeometryData_Line(XYZAxis axis, Effect*effect, class Effect*line, GizmoType gizmoType);
	~GeometryData_Line();
...
class GeometryData_Box :public GeometryData
{
public:
	GeometryData_Box(XYZAxis axis, Effect*effect, class Effect*line, GizmoType gizmoType);
	~GeometryData_Box();
