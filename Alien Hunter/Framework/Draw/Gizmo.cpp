...
#pragma endregion
		...
		localForward = selection[0]->Direction();
...
void Gizmo::SetSelectionPool(class IObject *entity)
{
	selectionPool.push_back(entity);
}

void Gizmo::Erase(IObject * entity)
{
	vector<IObject*>::iterator viter;

	viter = selectionPool.begin();

	for (; viter != selectionPool.end(); viter++)
	{
		if ((*viter) == entity)
		{
			selectionPool.erase(viter);
			break;
		}
	}
}
...
void Gizmo::PickObject()
{
	...
	{
		IObject * entity = selectionPool[i];

		if (entity->IsPicked())
		{
			bool b = false;
			for (IObject * find : selection)
			{
				if (find == entity)
				{
					b = true;
...
void Gizmo::SetGizmoPosition()
{
	switch (activePivot)
	{
		...
		case Gizmo::EPivotType::SelectionCenter:
		{
			...
			for (IObject * selected : selection)
			{
				D3DXVECTOR3 pos;
