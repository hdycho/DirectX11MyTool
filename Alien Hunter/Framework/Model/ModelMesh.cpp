...
void ModelMesh::RenderInstance(UINT count, UINT tec)
{
	UINT stride = sizeof(VertexTextureNormalTangentBlend);
	UINT offset = 0;
	D3D::GetDC()->IASetVertexBuffers(0, 1, &vertexBuffer, &stride, &offset);
...
{
	CsResource::CreateRawBuffer(sizeof(VertexTextureNormalTangentBlend)*vertexCount, vertices, &vertexBuffer);
	CsResource::CreateRawBuffer(sizeof(D3DXMATRIX)*64, NULL, &instanceBuffer);
}