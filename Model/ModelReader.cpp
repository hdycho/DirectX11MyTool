...
void Model::BindingMesh()
{
	...
	for (ModelMesh*mesh : meshes)
	{
		...
		for (ModelMeshPart*part : mesh->meshParts)
		{
			...
			{
				if (part->vertices[i].Position.x > maxX)
					maxX = part->vertices[i].Position.x;
				if (part->vertices[i].Position.x < minX)
					minX = part->vertices[i].Position.x;
			}
...
void Model::SetBoundingPos()
{
	for (int i = 0; i < meshes.size(); i++)
	{
		for (int j = 0; j < meshes[i]->meshParts.size(); j++)
		{
